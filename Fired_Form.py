...
def FT_data_entry(part_info_vec,key,entry_root):
    #********************************************************************************************
    ...
    #********************************************************************************************
    entry_root.withdraw()
    global FT_data_entry_root
    global frame_ID
    global FT_specs
    start_time = time.time()
    FT_data_entry_root = CTKlib.MultiFrameWindow()
    FT_data_entry_root.title('Fired Dimension Entry Form')
    frame_ID, FT_specs, part_info_vec = acc.verified_Fired_dimension_info(part_info_vec)
    try:
        if key == 'Edit Entry':
            orignal_data = acc.read_FT_database(part_info_vec)
            OD_L_ = orignal_data[3:13]
            Width_ = orignal_data[13:23]
            Thickness_ = orignal_data[23:33]
            ID1_ = orignal_data[33:43]
            ID2_ = orignal_data[43:53]
            ID3_ = orignal_data[53:63]
            Warpage_ = orignal_data[63:73]
        else:
            OD_L_ = ['','','','','','','','','','']
            Width_ = ['','','','','','','','','','']
            Thickness_ = ['','','','','','','','','','']
    ...
    def part_info_label(current_frame_ID,part_num,rev,mo_num):
        ...
        rounded_box = customtkinter.CTkFrame(CTKlib.MultiFrameWindow.frames[current_frame_ID], width=box_width, height=90, corner_radius=10, fg_color='#4818f8')
        rounded_box.place(relx=0.01, rely=0.01)

        # Position the labels inside the rounded box
        label_pn = customtkinter.CTkLabel(rounded_box, text=f'Part Number \u25BA {part_num}', fg_color="#4818f8", font=("Lufga Bold", 16,'bold'))
        label_pn.place(relx=0.05, rely=0.05)
    
        label_rev = customtkinter.CTkLabel(rounded_box, text=f'Revision \u25BA {rev}', fg_color="#4818f8", font=("Lufga Bold", 16,'bold'))
        label_rev.place(relx=0.05, rely=0.3)
       
        label_mo = customtkinter.CTkLabel(rounded_box, text=f'MO Number \u25BA {mo_num}', fg_color="#4818f8", font=("Lufga Bold", 16,'bold'))
    ...
    def header_info(current_frame_ID, data_min, data_max):  
        ...
        label1 = customtkinter.CTkLabel(CTKlib.MultiFrameWindow.frames[current_frame_ID], text=f'Fired Part {current_frame_ID}', fg_color="#2e353e", font=("Lufga Bold", 50, "bold"))
        label1.pack(pady=30)
    ...
    for i in range(len(frame_ID)):
        CTKlib.MultiFrameWindow.create_nav(FT_data_entry_root,frame_ID[i],Hover_color)
        if frame_ID[i] == 'OD':
            header_info(frame_ID[i],FT_specs[2],FT_specs[3])
            ...
            part_info_label(frame_ID[i],part_info_vec[0],part_info_vec[2],part_info_vec[1])
            header_info(frame_ID[i],FT_specs[4],FT_specs[5])
            # Generate entry boxes
            text = 'Width'
            # Width = gen_box_entry(frame_ID[i],text)
            Width = gen_box_entry(frame_ID[i], text, Width_)
        elif frame_ID[i] == 'Thickness':
            header_info(frame_ID[i],FT_specs[6],FT_specs[7])
            part_info_label(frame_ID[i],part_info_vec[0],part_info_vec[2],part_info_vec[1])
             # Generate entry boxes
            text = 'Thickness'
            # Thickness_w = gen_box_entry(frame_ID[i],text)
            Thickness = gen_box_entry(frame_ID[i],text,Thickness_)
        elif frame_ID[i] == 'ID A':
            header_info(frame_ID[i],FT_specs[8],FT_specs[9])
            part_info_label(frame_ID[i],part_info_vec[0],part_info_vec[2],part_info_vec[1])
            # Generate entry boxes
            text = 'ID A'
            ...
            def submit():
                try:
                    ...
                    FT_dim_output.append(box_val_list)
                    ...
                    box_val_list = []
                    for box in Thickness:  # Iterate over each entry box in the list
                        bx = box.get()  # Retrieve value from each entry box separately
                        box_val_list.append(bx)
                    FT_dim_output.append(box_val_list)
                    ...
                    index = 0
                    index_2 = 1
                    for sublist in FT_dim_output:
                        for element in sublist:
                            if double_entry(element) == True:
                                ident = FT_box_identify(index)
                                pop_up_root = CTKlib.PopUpWindow()
                    ...
                    #====================================================================
                        ...
                        end_time = time.time()
                        if key == 'Edit Entry':
                            acc.edit_FT_database_table(part_info_vec,FT_dim_output,start_time,end_time) # Write to the Database
                        else:
                            acc.write_to_FT_database(part_info_vec,FT_dim_output,start_time,end_time)
                        Sucess.sucess_func()
                        FT_data_entry_root.destroy()
                except TclError as e:
                    print(f'{e}')
                except Exception as e:
                    pop_up_root = CTKlib.PopUpWindow()
